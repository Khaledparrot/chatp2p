...
LOGIN_URL = r"https://algeria.blsspainglobal.com/DZA/account/login"
...
def is_captcha_limit_reached(page):
    return page.evaluate(r"() => document.body.innerText.includes('maximum number of allowed captcha submissions')")
...
def load_random_user_agent():
    """Load a random user-agent from users.txt."""
    try:
        with open("users.txt", "r", encoding="utf-8") as f:
            agents = [line.strip() for line in f if line.strip()]
        return random.choice(agents) if agents else None
    except Exception as e:
        print("Error loading user-agents:", e)
        return None
    

def toggle_airplane_mode(wait=10):
    """Toggle airplane mode using adb."""
    print("✈️ Enabling airplane mode...")
    subprocess.run("adb shell cmd connectivity airplane-mode enable", shell=True)
    time.sleep(wait)
    print("📶 Disabling airplane mode...")
    subprocess.run("adb shell cmd connectivity airplane-mode disable", shell=True)
    time.sleep(wait)

def handle_too_many_requests(page):
    """Check for 'Too Many Requests' error and reload with a new User-Agent if present."""
    if page.locator(r"h1:has-text('Too Many Requests')").is_visible() or \
       page.locator(r"h1:has-text('Application Temporarily Unavailable')").is_visible() or \
       page.locator(r"h1:has-text('403 Forbidden')").is_visible():
        
        print("Detected 'Too Many Requests'. Reloading with new User-Agent...")
        print("🚫 Detected block: Too Many Requests or 403 Forbidden.")
        print("🌐 Toggling airplane mode to change IP...")

        toggle_airplane_mode(wait=10)
...
def fill_login(page):
    ...
    print(" Logging in...")
    login_input = page.evaluate_handle(r'''() => {
...
def click_book_now(page):
    ...
    check_and_solve_captcha(page)
    try:
        button = page.query_selector(r'a[href="/DZA/appointment/newappointment"]')
        if button:
            button.click()
...
function finalSubmitClick(maxTries = 30, interval = 500) {
  let tries = 0;
  ...
  function tryClick() {
    const btn = Array.from(document.querySelectorAll('button.btn.btn-primary'))
    ...
    if (btn && isVisible(btn)) {
      ...
      if (tries < maxTries) {
        tries++;
        console.log("Waiting for Submit button... Try:", tries);
        setTimeout(tryClick, interval);
      } else {
        console.warn("Submit button not found or not visible after multiple attempts.");
      }
    }
  }
...
"""
        time.sleep(10)
        # check_and_solve_captcha(page)
        page.on("console", lambda msg: print("JS LOG:", msg.text))
...
with sync_playwright() as p:
    ...
    page.goto(LOGIN_URL, wait_until="load")  # Wait for full page load (including JS)
    human_wait(6, 10)

    while True:
        try:
            # Wait until the page is fully loaded and DOM settled
            page.wait_for_load_state("load", timeout=15000)  # Wait for full load again after redirects/actions
            time.sleep(2)  # Small buffer after load for JS rendering
            ...
            if is_password_required(page):
                print("Champ de mot de passe détecté après captcha.")
                fill_password(page)
                time.sleep(3)
                continue
